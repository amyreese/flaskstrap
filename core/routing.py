...
class context(object):
    """Prefix all routing and template paths with the given base URL for any
    view methods defined during the scope of this context. This can also be
    nested, and all contexts will be concatenated in the order they were
    entered. Usage example:
    ...
    This creates two routing endpoints: one that routes /foo to index(), and
    one that routes /foo/bar to bar()."""
...
def api(api_url, methods=['GET', 'POST', 'PUT'], format='application/json',
    ...
    def decorator(f):
        ...
        if defaults is not None:
            ...
            m = len(args)
            n = len(defaults)
            args[m-n:m] = ["{0}={1}".format(a, d) for a, d
                           in zip(args[m-n:], defaults)]
        ...
        def decorated_function(*args, **kwargs):
            ...
            if request.method == 'GET' or request.method == 'DELETE':
                ...
                elif request.content_type == form_type:
                    payload = {}
